                            ...
                            <div class="row">
                                ...
                                <div class="col-2">
                                    <div class="form-group">
                                        ...
                                        <select class="select2 form-control" style="width: 100%;" id="product_type" onchange="getResult()">
                                ...
                                <div class="col-md-3">
                                    Total Sale Quantity:
                                    <input type="text" value="" id="total_quantity" readonly /><b> P</b>
                                </div>
...
<script>
    function getResult() {
        ...
        $.ajax({
            type: 'POST',
            url: '<?= base_url('/search/productreport') ?>',
            data: {
                from_date: from_date,
                to_date: to_date,
                user_code: user_code,
            ...
            success: function(data) {
                ...
                var totalPaid = 0;
                var totalDue = 0;
                // var quantitiesArray = item.quantities.split(','); 
                // var totalQuantityPerItem = 0;
                // quantitiesArray.forEach(function(quantity) {
                //     totalQuantityPerItem += parseFloat(quantity);
                // });
                table.empty();
                if (data.length > 0) {
                    var headerRow = "<tr><th>User Code</th><th>Customer</th><th>Date</th><th>Shift</th><th>Product Type</th><th>Qty.</th><th>Per Price</th><th>Amount</th><th>Discount</th><th>Total</th><th>Paid Amount</th><th>Due Amount</th></tr>";
                    table.append(headerRow);

                    data.forEach(function(item) {
                        var row = "<tr><td>" + item.user_code + "</td><td>" + item.user_name + "</td><td>" + item.date + "</td><td>" + item.shift_name + "</td><td>" + item.item_names + "</td><td>" + item.quantities + "</td><td>" + item.per_prices + "</td><td>" + item.prices + "</td><td>" + item.discount + "</td><td>" + item.total_amount + "</td><td>" + item.paid_amount + "</td><td>" + item.due_amount + "</td></tr>";
                        table.append(row);
